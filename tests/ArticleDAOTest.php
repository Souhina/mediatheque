...
class ArticleDaoTest extends TestCase{
    ...
    protected function setUp(): void{
        //createMock() crée un faux objet qui a toutes les methodes 
    ...
    //Ce tableau simule ce que MySQL renvoerrait pour un livre
    $ligneSimulee = [
        'id' => 1,
        'type' => 'livre',
        'titre' => '1984',
    ...
    // - fetch() renvoienotre ligne simulée
    ...
    $this->assertEquals('Georges Orwell', $article->getAuteur());
    ...
    //On simule deux lignes renvoyés par mySQL 
    $lignesSimulees = [[
        'id' => 1, 'type' => 'livre',
    ...
    public function testCreateRetourneLIdInsere(): void{
        //lastInsertId() renvoie l'id attribué par MySQL aprés l'INSERT
        $this->stmtMock->method('execute')->willReturn(true);
        $this->pdoMock->method('prepare')->willReturn($this->stmtMock);
    ...
    //Test de la methode hydrate() via find
    ...
    public function testHydrateAssigneBienLIdDepuisLaBDD(): void{
        ...
        //L'id doit etre celui qui vient de la base, pas null
